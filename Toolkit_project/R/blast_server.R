...
blast_server <- function(input, output, session) {
  ...
  output$protein_select_ui <- renderUI({
    req(proteins())
    selectInput("protein_id", "Select protein:", choices = proteins())
  })
  ...
  output$plot_settings <- renderUI({
    ...
    if (input$plot_type == "hist") {
      selectInput("hist_var", "Select variable for histogram:", choices = num_vars)
    } else if (input$plot_type == "scatter") {
      tagList(
        selectInput("scatter_x", "X axis:", choices = num_vars, selected = num_vars[1]),
        selectInput("scatter_y", "Y axis:", choices = num_vars, selected = num_vars[min(2, length(num_vars))])
      )
    } else if (input$plot_type == "boxplot") {
      selectInput("box_var", "Select variable for boxplot:", choices = num_vars)
    }
  })
