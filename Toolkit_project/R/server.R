...
function(input, output, session) {
  ...
  load_builtin <- function(name) {
      ...
      read.csv("TPMs_table_100genes.csv", row.names = 1, check.names = FALSE)
    } else {
      NULL
    }
  }
  ...
  selected_data <- reactive({
    if (input$file_choice == "TPMs_table_100genes") {
      load_builtin(input$file_choice)
  ...
  output$var_select <- renderUI({
    ...
    selectInput("vars", "Select variables (columns) for analysis:",
  ...
  data_filtered <- reactive({
    ...
    # If user did not select any column, return NULL
  ...
  output$ma_samples <- renderUI({
    ...
    tagList(
      selectInput("ma_sample1", "Sample 1:", choices = colnames(df), selected = colnames(df)[1]),
      selectInput("ma_sample2", "Sample 2:", choices = colnames(df), selected = colnames(df)[2])
    )
  })
  ...
  observe({
    ...
    if (is.null(df) || nrow(df) == 0) {
      updateSelectInput(session, "gene", choices = character(0))
    } else {
      updateSelectInput(session, "gene", choices = rownames(df))
    }
  })
